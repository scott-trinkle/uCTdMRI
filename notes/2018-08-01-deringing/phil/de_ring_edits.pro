...
;Radial Median Filter - initialized to zeros
med_r = fltarr(nr, nt)
...
if(keyword_set(step)) then begin
  ...
  ;; Defines three copies of the median filter (a,b,c) for
  ;; three different widths / regions:
  ...
  ;; Copies the relevant regions into a single filter, mid_r
...
;; ^ var makes it a 2D median filter
;; neither, filters the whole thing as 1d with w_rd
